...
dymo.label.framework.Label.prototype._getAddressObjectText = function(e) {
    ...
    if (this.isDCDLabel())
        for (var t = dymo.xml.getElement(e, "FormattedText"), r = dymo.xml.getElements(t, "LineTextSpan"), g = 0; g < r.length; g++)
            for (var n = dymo.xml.getElements(r[g], "TextSpan"), g = 0; g < n.length; g++) {
...
dymo.label.framework.Label.prototype._setAddressObjectText = function(e, o) {
    if (this.isDCDLabel()) {
        ...
        !t || (t = dymo.xml.getElements(t[0], "TextSpan")) && (t = dymo.xml.getElement(t[0], "Text"),
        dymo.xml.setElementText(t, o))
    } else {
        var r, g = dymo.xml.getElement(e, "StyledText"), n = this._getStyledTextLineAttributes(g), e = dymo.xml.getElement(e, "LineFonts"), i = [];
        0 == (i = e ? dymo.xml.getElements(e, "Font") : i).length && (r = dymo.xml.parse('<Font Family="Arial" Size="12" Bold="False" Italic="False" Underline="False" Strikeout="False" />').documentElement);
        var s = dymo.xml.parse('<ForeColor Alpha="255" Red="0" Green="0" Blue="0" />').documentElement;
        dymo.xml.removeAllChildren(g);
        for (var a = o.split("\n"), l = 0; l < a.length; l++) {
            var u = a[l].replace("\r", "");
            l < a.length - 1 && (u += "\n");
            var c = r;
            0 < i.length ? c = l < i.length ? i[l] : i[i.length - 1] : 0 < n.length && (c = l < n.length ? dymo.xml.getElement(n[l], "Font") : dymo.xml.getElement(n[n.length - 1], "Font"));
            var d = s;
